...
import models
#import acerAgent
import acerAgent_on_policy as acerAgent
...
def main():    
    ...
    inner_replay_buffer = acerAgent.EpisodicReplayMemory(BUFFER_CAPACITY, MAX_EPISODE_LENGTH)
    replay_buffer = acerAgent.EpisodicReplayMemory(BUFFER_CAPACITY, MAX_EPISODE_LENGTH)
    agent = acerAgent.ACERAgent(model, model_optim, feature_encoder_optim,  GAMMA, ENTROPY_WEIGHT, CLASS_NUM, inner_replay_buffer, replay_buffer, device)
    ...
    for episode in range(EPISODE):
        ...
        for meta_batch in range(META_BATCH_RANGE):
            ...
            for inner_batch in range(INNER_BATCH_RANGE):
                ...
                inner_env = acerAgent.env(env_states_list, env_labels_list)
            ...
            meta_env = acerAgent.env(meta_env_states_list, meta_env_labels_list)
            agent.train(meta_env, inner_update=False, loss_list=losses)
        ...
        if (episode + 1) % 50 == 0:
            ...
            print(f"episode : {episode+1}, mean_loss : {mean_loss}")
            ...
            for i in range(TEST_EPISODE//ENV_LENGTH):
                ...
                for env in range(ENV_LENGTH):
                    ...
                    for batches, batch_labels in batch_dataloader:
                        batches, batch_labels = batches.to(device), batch_labels.to(device) 
                        total_num_of_test_samples += len(batch_labels)
                        
                        inner_sample_features = feature_encoder(sample_images)            
                        inner_sample_features = inner_sample_features.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 19, 19)
                        inner_sample_features = torch.sum(inner_sample_features, 1).squeeze(1)
                ...
                test_env = acerAgent.env(env_states_list, env_labels_list)
