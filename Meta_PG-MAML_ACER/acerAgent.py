...
class ACERAgent:
    ...
    def off_policy_inner_update(self, batch_size, replay_ratio=4):
    ...
    def train(self, env, inner_update=False, loss_list = None, off_policy_loss_list = None):
        ...
        if inner_update:
            self.compute_acer_loss(policies, q_values, values, actions, rewards, retrace, masks, policies, loss_list=off_policy_loss_list, inner_update=inner_update)
        else:
            self.compute_acer_loss(policies, q_values, values, actions, rewards, retrace, masks, policies, loss_list=off_policy_loss_list, inner_update=inner_update)
