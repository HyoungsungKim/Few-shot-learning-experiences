...
class PPOAgent:
    ...
    def select_action(self, model, states, labels):
        dist, critic_logit = model(states)
    ...
    def ppo_update(self, model, loss_list, states, labels, actions, log_probs, returns, values_t, clip_param):
        for state, label, action, log_prob, return_, value in zip(states, labels, actions, log_probs, returns, values_t):
            dist, critic_logit = model(state)
            _, predict_labels = torch.max(critic_logit.data, 1)
            assert len(predict_labels) == len(label)
            predicted_value = [1 if predict_labels[j] == label[j] else 0 for j in range(len(label))]
            predicted_value_t = torch.tensor([predicted_value], dtype=torch.float, requires_grad=True).to(self.device)
            ...
            #egularizer = critic_logit.pow(2).sum() / (2.0 * critic_logit.size(0))
            value_loss = nn.MSELoss()(return_, predicted_value_t) * 0.5 #+ regularizer
    ...
    def train(self, env, model, loss_list = None, clip_param = 0.2):
        ...
        while True:
            ...
            for step in range(min(env_slice, len(env))):
                actions = self.select_action(model, states, labels)
            ...
            if next_states is not None:
                _, next_critic_logit = model(next_states)
            ...
            # TODO : Check length of environment and length of reward_list are same
            ...
            self.ppo_update(model, loss_list, states_t, labels_t, actions_t, log_prob_t, returns_t, values_t, clip_param)
