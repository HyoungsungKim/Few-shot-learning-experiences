'''
Decay : 10000
Do not backpropagate feature weight
Reimplement innerloop of MAML based on https://github.com/katerakelly/pytorch-maml/blob/master/src/inner_loop.py
Does not work
'''
import torch
...
import os
#import models
import models_with_linear as models
...
def main():    
    ...
    for episode in range(EPISODE):
        ...
        for meta_batch in range(5):
            ...
            #relation_fast_weights = OrderedDict((name, param) for (name, param) in relation_network.named_parameters())
            ...
            for inner_batch in range(5):
                ...
                #print(sample_labels)
                #print(feature_encoder.get_linear(inner_sample_features))
                #print(feature_encoder.get_linear(inner_sample_features).view(-1, CLASS_NUM))
                #assert 1 == 2
                
                inner_sample_loss = cross_entropy(feature_encoder.get_linear(inner_sample_features), sample_labels)
                ...
                inner_relation_pairs = torch.cat((inner_sample_feature_ext, inner_batch_features_ext), 2).view(-1, FEATURE_DIM * 2, 19, 19).detach()
                ...
                inner_feature_gradients = torch.autograd.grad(inner_sample_loss, feature_fast_weights.values(), create_graph=True, allow_unused=True)
            ...
            sample_loss = cross_entropy(feature_encoder.get_linear(sample_features), sample_labels)
            ...
            relation_pairs = torch.cat((sample_features_ext, batch_features_ext), 2).view(-1, FEATURE_DIM * 2, 19, 19).detach()
            ...
            feature_gradients = torch.autograd.grad(sample_loss, feature_fast_weights.values(), create_graph=True, allow_unused=True)
            ...
            relation_name_grads = {name: g for ((name, _), g) in zip(relation_network.named_parameters(), relation_gradients)}
