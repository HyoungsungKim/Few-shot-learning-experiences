...
def main():
    ...
    for test_iter in range(5):
        ...
        for i in range(TEST_EPISODE):
            ...
            test_feature_encoder_optim = torch.optim.Adam(feature_encoder.parameters(), lr=LEARNING_RATE)
            test_relation_network_optim = torch.optim.Adam(relation_network.parameters(), lr=LEARNING_RATE)
            for inner_batch in range(50):
                inner_num_of_query_image = 5
                inner_sample_per_class = 15
                task = tg.MiniImagenetTask(metatest_character_folders, CLASS_NUM, inner_sample_per_class * 6, inner_num_of_query_image)
                sample_dataloader = tg.get_mini_imagenet_data_loader(task, num_per_class=inner_sample_per_class, split="train", shuffle=False)                
                ...
                inner_sample_features = inner_sample_features.view(CLASS_NUM, inner_sample_per_class, FEATURE_DIM, 19, 19)
                ...
                test_feature_encoder_optim.zero_grad()
                test_relation_network_optim.zero_grad()
                
                inner_loss.backward()
                
                test_feature_encoder_optim.step()
                test_relation_network_optim.step()
