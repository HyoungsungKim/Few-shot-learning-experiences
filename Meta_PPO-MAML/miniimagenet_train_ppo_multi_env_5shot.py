...
def main():    
    ...
    critic = models.Critic(FEATURE_DIM, RELATION_DIM, CLASS_NUM)
    ...
    critic_optim = torch.optim.Adam(critic.parameters(), lr= 2.5 * LEARNING_RATE * 10)
    ...
    for episode in range(EPISODE):
        ...
        for meta_batch in range(META_BATCH_RANGE):
            ...
            actor_fast_weight = OrderedDict(actor.named_parameters())
            critic_fast_weight = OrderedDict(critic.named_parameters())
            for inner_batch in range(INNER_BATCH_RANGE):
                ...
                log_prob_list, actions_list, states_list, labels_list, target_value_list, advantages_list = agent.train(actor, critic, inner_env)
                for old_log_prob, actions, states, labels, target_value, advantages in zip(log_prob_list, actions_list, states_list, labels_list, target_value_list, advantages_list):
                    dist = actor(states)
                    ...
                    predicted_value_t = torch.tensor(predicted_value, dtype=torch.float, requires_grad = True).to(device)
                    ...
                    inner_actor_loss = -torch.min(surr1, surr2).mean() - ENTROPY_WEIGHT * entropy
                    inner_critic_loss = nn.MSELoss()(target_value, predicted_value_t) + nn.CrossEntropyLoss()(critic_logit, labels)
                        
                    #inner_actor_loss = torch.stack(inner_policy_losses).mean()
                    #inner_critic_loss = torch.stack(inner_value_losses).mean()
                    
                    inner_actor_gradients = torch.autograd.grad(inner_actor_loss, actor_fast_weight.values(), create_graph=True, allow_unused=True)
                    inner_value_gradients = torch.autograd.grad(inner_critic_loss, critic_fast_weight.values(), create_graph=True, allow_unused=True)
                    
                    actor_fast_weight = OrderedDict(
                        (name, param - INNER_LR * (0 if grad is None else grad))                    
                        for ((name, param), grad) in zip(actor_fast_weight.items(), inner_actor_gradients)                    
                    ) 
                    
                    critic_fast_weight = OrderedDict(
                        (name, param - INNER_LR * (0 if grad is None else grad))                    
                        for ((name, param), grad) in zip(critic_fast_weight.items(), inner_value_gradients)                    
                    )   
            
            actor.weight = actor_fast_weight
            critic.weight = critic_fast_weight
            ...
            log_prob_list, actions_list, states_list, labels_list, target_value_list, advantages_list = agent.train(actor, critic, meta_env)
            for old_log_prob, actions, states, labels, target_value, advantages in zip(log_prob_list, actions_list, states_list, labels_list, target_value_list, advantages_list):
                dist = actor(states)
                critic_logit = critic(states)
                _, predict_labels = torch.max(critic_logit.data, 1)
                ...
                actor_loss = -torch.min(surr1, surr2).mean() - ENTROPY_WEIGHT * entropy
                critic_loss = nn.MSELoss()(target_value, predicted_value_t) + nn.CrossEntropyLoss()(critic_logit, labels)
                ...
                meta_batch_actor_loss = actor_loss
                meta_batch_critic_loss = critic_loss
                ...
                meta_batch_critic_loss.backward(retain_graph=True)
        ...
        if (episode + 1) % 500 == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                rewards = agent.test(actor, test_env)
