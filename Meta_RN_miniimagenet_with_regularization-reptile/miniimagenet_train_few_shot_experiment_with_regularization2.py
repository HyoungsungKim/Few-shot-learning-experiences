...
def main():    
    ...
    for episode in range(EPISODE):
        ...
        for meta_batch in range(META_BATCH_SIZE):
            ...
            for inner_batch in range(INNER_ITER):
                task = tg.MiniImagenetTask(metatrain_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS, NUMBER_OF_QUERY)
                sample_dataloader = tg.get_mini_imagenet_data_loader(task, num_per_class=SAMPLE_NUM_PER_CLASS, split="train", shuffle=False)                
                batch_dataloader = tg.get_mini_imagenet_data_loader(task, num_per_class=NUMBER_OF_QUERY, split="test", shuffle=True)    
                ...
                inner_sample_feature_ext = inner_sample_features.unsqueeze(0).repeat(NUMBER_OF_QUERY * CLASS_NUM, 1, 1, 1, 1)
            ...
            # * https://github.com/openai/supervised-reptile/blob/master/supervised_reptile/train.py#L55
            # meta_step_size_final is 0
            # meta_step_size = 1
            ...
            sample_features_ext = sample_features.unsqueeze(0).repeat(NUMBER_OF_QUERY * CLASS_NUM, 1, 1, 1, 1)
        ...
        if (episode + 1) % 500 == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                task = tg.MiniImagenetTask(metatest_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS, EVALUATE_SHOT)
                sample_dataloader = tg.get_mini_imagenet_data_loader(task, num_per_class=SAMPLE_NUM_PER_CLASS, split="train", shuffle=False)                
                test_dataloader = tg.get_mini_imagenet_data_loader(task, num_per_class=EVALUATE_SHOT, split="test", shuffle=True)
                ...
                sample_features_ext = sample_features.unsqueeze(0).repeat(EVALUATE_SHOT * CLASS_NUM, 1, 1, 1, 1)
